...
LRApp.controller('ExampleController', function ($scope) {
	...
	$scope.init = function () {
		...
		$scope.LRModel = new Regression.LinearRegression(0, .01, .01, $scope.iterations);
	...
	$scope.retrain = function () {
		$scope.LRModel = new Regression.LinearRegression(0, $scope.theta, $scope.alpha, $scope.iterations);
		$scope.LRModel.train($scope.features, $scope.targets);
	};
